class Calendar
{
	constructor(id)
	{
		...
		this.selected_day = this.displayed_date
			...
			for (let k = 0; k < 7; ++k)
			{
				...
				else
				{
					...
					if (are_real_date_displayed)
					{
						...
						if (this.selected_day.getDate() == days_array[i].number)
	...
	moveLeft()
	{
		this.displayed_date = new Date( //set the day to prev month
			this.displayed_date.getFullYear(), this.displayed_date.getMonth() - 1, 1
		)
	...
	selectHandler(e)
	{
		...
		if (this.selected_day_node)
			this.selected_day_node.classList.remove('calendar-cell-selected')
		...
		this.selected_day_node = e.target
		e.target.classList.toggle('calendar-cell-selected')
	}
}